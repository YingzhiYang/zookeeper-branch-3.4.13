...
public abstract class ServerCnxnFactory {
    ...
    //abstract转到实现类NIOServerCnxnFactory
    ...
    //抽象类，跳转到实现类NIOServerCnxnFactory.startup()
    ...
    static public ServerCnxnFactory createFactory() throws IOException {
        String serverCnxnFactoryName =
            System.getProperty(ZOOKEEPER_SERVER_CNXN_FACTORY);//取默认属性"zookeeper.serverCnxnFactory"
        if (serverCnxnFactoryName == null) {//如果没有配置，那么也是默认取NIOServer这个类
            serverCnxnFactoryName = NIOServerCnxnFactory.class.getName();//这里也可以切换成Netty使用：NettyServerCnxnFactory.class.getName();
        }
        try {//所以在这里就构造出来了一个NIOServerCnxnFactory的对象用来实现ServerCnxnFactory
