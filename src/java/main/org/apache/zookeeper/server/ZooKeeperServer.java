...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public void loadData() throws IOException, InterruptedException {
        ...
        if(zkDb.isInitialized()){
            ...
            //没有初始化，加载数据zkDb.loadDataBase()，进入
    ...
    public void takeSnapshot(){
        ...
        try {
            //这里直接save一个database的Datatree，我们进入save
    ...
    public synchronized void startup() {
        //sessionTracker：Session追踪器，用来追踪session是不是过期了，这里就是一个倒计时
        if (sessionTracker == null) {
            //如果没有，创建一个出来
        ...
        //开始运行session跟踪器
        startSessionTracker();
        //请求处理器，用来处理请求的，有多种类型，而且可能是链表的形式，从p1.next=p2这样，用不同的处理器执行不同的逻辑。
    ...
    protected void setupRequestProcessors() {
        ...
        //第一个处理器是PrepRequestProcessor，第二个就是syncProcessor
        //PrepRequestProcessor.next=syncProcessor.next=FinalRequestProcessor
        //服务端收到客户端的socket以后取出其中的packet然后存到queue中，再由firstProcessor.run()从queue取出request，
        //请求处理器链表来处理这个request，这个queue就是PrepRequestProcessor类中的linkedqueue：submittedRequests
        //最后再由FinalRequestProcessor把结果返回出去，这里其实和客户端是NIOServerCnxnFactory一样的逻辑，只不过是反向的
        //客户端的是：Requet-包装成->Packet-放到->OutgoingQueue->通过socket发送出去
        //服务端的是：从socket-取出->packet-转换成->request-放入->queue里
        //但是这里要记得，这之前我们一直在运行的都是NIOServerCnxnFactory线程
        firstProcessor = new PrepRequestProcessor(this, syncProcessor);
        //这里启动的是PrepRequestProcessor类，所以我们要去PrepRequestProcessor的run()里面看下都有什么
    ...
    protected void startSessionTracker() {
        //这里强转以后调用start方法，说明SessionTrackerImpl也是一个线程类，进入找到run方法
    ...
    public void submitRequest(Request si) {
        //firstProcessor出现了，这个在刚才setupRequestProcessors()方法里已经初始化好了的PrepRequestProcessor对象
        if (firstProcessor == null) {
            synchronized (this) {
                try {
                    ...
                    // after the setup.
                    while (state == State.INITIAL) {//firstProcessor如果正在初始化，那就等着初始化完毕
                        wait(1000);
                    }
        ...
        try {//到这里
            touch(si.cnxn);
            boolean validpacket = Request.isValid(si.type);//判断request的类型是不是合法的，注意si就是request的对象
            if (validpacket) {
                //判断完毕以后到这里来first是PrepRequestProcessor对象，进入找到PrepRequestProcessor类的实现方法
    ...
    public void processPacket(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        RequestHeader h = new RequestHeader();//构造请求头，这里在客户端似曾相识对吧
        ...
        //根据header的不同类型进行处理
        if (h.getType() == OpCode.auth) {//这个auth其实就是命令，addauth命令，往下
            LOG.info("got auth packet " + cnxn.getRemoteSocketAddress());
            AuthPacket authPacket = new AuthPacket();//创建一个实例
            ByteBufferInputStream.byteBuffer2Record(incomingBuffer, authPacket);//从incomingBuffer中取出并反序列化出来
            String scheme = authPacket.getScheme();
            //根据传入的不同digest 或者ip生成不同的实例，进入看下，
            // 发现有三种DigestAuthenticationProvider，IPAuthenticationProvider和SASLAuthenticationProvider会根据传入来
            // 生成不同的实例，比如我们传入的是digest则生成DigestAuthenticationProvider实例
            AuthenticationProvider ap = ProviderRegistry.getProvider(scheme);
            Code authReturn = KeeperException.Code.AUTHFAILED;
            if(ap != null) {
                try {
                    //这里调用生成的实例，以digest为例，我们去DigestAuthenticationProvider实现方法里面看
            ...
            if (h.getType() == OpCode.sasl) {//也不是sasl命令
                ...
                //直接从socket里面拿出一个request，记得这个cnxn，传入的是this
