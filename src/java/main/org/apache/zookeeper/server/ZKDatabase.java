...
public class ZKDatabase {
    ...
    protected FileTxnSnapLog snapLog; //事务快照的工具类
    ...
    public long loadDataBase() throws IOException {
        //这里snapLog就是刚才说的工具类FileTxnSnapLog，看restore()，我们这里传入的dataTree就是我们打的快照的内容，
        // 要记得服务器接收到请求第一步就是创建事务日志持久化，所以持久化是先于快照的
        //那么我们设想这样一个场景：比如说我们的快照到事务4，但是我们实际持久化运行的已经到了8，服务器挂了，
        // 那么我们的dataTree里面记录的Zxid，也只能就是4，那么我们的5-8该怎么办呢？我们进入这个restore看看里面是什么
    ...
    public void addCommittedProposal(Request request) {
        ...
        try {
            wl.lock();
            if (committedLog.size() > commitLogCount) {//commitLogCount是500
                //如果超过500就把第一个remove保证committedLog里面只有500个事务
                committedLog.removeFirst();
                minCommittedLog = committedLog.getFirst().packet.getZxid();//更新最小的事务id
            }
            if (committedLog.size() == 0) {
                minCommittedLog = request.zxid;//存下最小的事务id
                maxCommittedLog = request.zxid;//存下最大的事务id
            }
            ...
            p.packet = pp;
            p.request = request;
            //把这个事务添加到，committedLog里面，从我们分析解析源码来看，这个committedLog就是存的已经持久化好的事务，
            // 也就是持久化，更新内存完毕以后存入这里的，因为我们已经到了FinalRequestProcessor只要到了这里就说明持久化
            // 已经做好了，而rc = zks.processTxn(hdr, txn)就是在更新内存，而后才执行的addCommittedProposal()这个方法，
            // 那么committedLog这个链表存的永远都是最新的事务合集
            committedLog.add(p);
    ...
    public boolean append(Request si) throws IOException {
        return this.snapLog.append(si);//又被加入snapLog这个工具类，进入
    }
