...
public class FileTxnLog implements TxnLog {
    ...
    public synchronized void rollLog() throws IOException {
        if (logStream != null) {//如果流不是空
            this.logStream.flush();//写入文件
            this.logStream = null;//相当于生成新文件
            oa = null;
        }
    }
    ...
    {   //抛开验证往下走
        ...
        if (logStream==null) {//看到一个关键的对象logStream， log流对象
           if(LOG.isInfoEnabled()){
                LOG.info("Creating new log file: " + Util.makeLogName(hdr.getZxid()));
           }

           logFileWrite = new File(logDir, Util.makeLogName(hdr.getZxid()));//logDir这个就是我们zoo.cfg配置的路径
           fos = new FileOutputStream(logFileWrite);//这里就会不断地往日志里面放东西，放的是什么呢，就是传入的TxnHeader hdr, Record txn 日志事务头和事务
           logStream=new BufferedOutputStream(fos);//但是一直到下面，返回的true都没有写文件的步骤，所以文件步骤一定在外面
    ...
    public synchronized void commit() throws IOException {
        if (logStream != null) {//如果logStream不是空就flush()写入进去
            logStream.flush();
        }
        for (FileOutputStream log : streamsToFlush) {
            log.flush();
