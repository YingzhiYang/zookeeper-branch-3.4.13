...
public class FileTxnSnapLog {
    ...
    public long restore(DataTree dt, Map<Long, Integer> sessions, 
        ...
        //snapLog快照的日志，把快照的日志反序列化出来数据，加载到DataTree中，deserialize里就是具体反序列化的内容
        snapLog.deserialize(dt, sessions);
        return fastForwardFromEdits(dt, sessions, listener);//进入，这个方法就是找事务并取出的
    }
    ...
    public long fastForwardFromEdits(DataTree dt, Map<Long, Integer> sessions,
        ...
        //取出全部的事务id，并做成一个iterator，用来还原已经执行的事务，dt.lastProcessedZxid这个是快照中记录的最大的
        // 事务id，这里为什么会加1呢？因为我们的日志里数据是不会消失的，加1是为了检测是否没有打到快照里但是应该被执行
        // 的日志，如果有这样的，就会走到下面processTransaction()重新执行并更新到DataTree里。
        // 比如我们一共有1-8个日志，到4这里持久化就中断了，所以只记录到了4(lastProcessedZxid=4)
        // 本来我们取出的itr应该是4-8，但是经过传入txnLog.read(lastProcessedZxid+1)的操作以后，
        // 这里我们取出来的itr里面的数据应该是5-8的数据。
        TxnIterator itr = txnLog.read(dt.lastProcessedZxid+1);
        long highestZxid = dt.lastProcessedZxid;//这里我们取出快照里面最大的Zxid
        TxnHeader hdr;
        try {
            while (true) {//一个一个的循环事务id，然后用processTransaction()执行所有的事务，那么第一次进来必定是事务5
                // iterator points to 
                ...
                hdr = itr.getHeader();
                if (hdr == null) {
                    //empty logs hdr是null说明取出的数据和快照一致，都是4，直接返回Zxid
                ...
                try {
                    //把没有执行的事务再此执行并更新内存返回给客户端(牢记打快照在更新内存之前，有了快照内存未必更新)
                ...
                //这里itr.next(),把自身的zxid更新为6，再循环一遍到7，8，达成条件break，跳出循环完成所有事务的更新
                if (!itr.next())
                    break;
            }
        } finally {
    ...
    public boolean append(Request si) throws IOException {
        return txnLog.append(si.hdr, si.txn);//最终被加入事务log的工具类txnLog中，进入
        //si.hdr，si.txn这里的两个参数，都是我们在PrepRequestProcessor中的pRequest2Txn()里面new出来的：request.txn = new CreateTxn(...)
    }
