...
public class QuorumPeerConfig {
    ...
    public void parse(String path) throws ConfigException {
        ...
        try {
            ...
            FileInputStream in = new FileInputStream(configFile);//把文件变成流文件处理
            try {
                cfg.load(in);//把文件流装载成为Properties类实例
            } finally {
                in.close();
            }
            //解析内容到Properties类中，进入
            parseProperties(cfg);
        } catch (IOException e) {
    ...
    throws IOException, ConfigException {
        ...
        for (Entry<Object, Object> entry : zkProp.entrySet()) {
            ...
            if (key.equals("dataDir")) {
            ...
            } else if (key.startsWith("server.")) {//这里会识别到server.开头的字符串，并以此决定是单机还是集群
                int dot = key.indexOf('.');
                ...
                if (parts.length > 3){
                    if (parts[3].toLowerCase().equals("observer")) {//识别配置文件中的observer字段
                        type = LearnerType.OBSERVER;
                    } else if (parts[3].toLowerCase().equals("participant")) {//识别配置文件中的participant字段
        ...
        if (servers.size() == 0) {
            ...
            if(serverGroup.size() > 0){
                ...
                quorumVerifier = new QuorumMaj(servers.size());//集群验证器，用来决定是不是过半，里面只有this.half = n/2;
            }

            // Now add observers to servers, once the quorums have been
            // figured out
            //最终把observers加到servers里面，由于前面已经做了处理，所以observers才不会被算在过半的选举中
            ...
            //再次验证observer，如果发现server.1=localhost:2887:3887:observer，
            // 如果发现myid对应的后面有关键字observer那么就把这个myid重新做成OBSERVER
            LearnerType roleByServersList = observers.containsKey(serverId) ? LearnerType.OBSERVER
                    : LearnerType.PARTICIPANT;
            //这里我们可以看到会更新peerType，说明observer是由配置server.1=localhost:2887:3887:observer决定的，
            // 而不是文件中peerType=observer决定的，即便配置成peerType=participant，后面host只要配置了observer，还是会被刷新掉的
