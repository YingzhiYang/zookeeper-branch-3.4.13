...
public class Follower extends Learner{
    ...
    void followLeader() throws InterruptedException {
        ...
        try {
            QuorumServer leaderServer = findLeader(); //首先找到哪个是leader，领导者选举结束后，这个方法就可以直接获得Leader
            try {
                connectToLeader(leaderServer.addr, leaderServer.hostname);//进入，关注这里connectToLeader
                long newEpochZxid = registerWithLeader(Leader.FOLLOWERINFO);//发送数据给leader，进入
                ...
                syncWithLeader(newEpochZxid);//同步数据，这里就会接收到Leader告诉我的同步方式和需要同步的数据，进入
