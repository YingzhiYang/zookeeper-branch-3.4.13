...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        loadDataBase();//这里和单机模式一样，从快照里导出数据到内存，进入看
        cnxnFactory.start(); //开启一个线程，接受客户端的请求
        startLeaderElection(); //领导者选举方法,进行领导者选举，确定服务器角色，在针对不同的服务器角色初始化
        super.start(); //这里就会启动QuorumPeer本身的run方法，因为QuorumPeer本身就是一个线程类
    }
    ...
    synchronized public void startLeaderElection() {
    	try {
    	    //首先生成投票，投给自己，因为这个myid是自己的myid，我们进入Vote看下
        ...
        for (QuorumServer p : getView().values()) {//这里是寻找本机地址的地方，端口就是我们配置的第一个port 2887这里的端口时进行服务器同步的
            if (p.id == myid) {
                myQuorumAddr = p.addr;
                break;
            }
        }
        if (myQuorumAddr == null) {//如果本机没有找到异常
            throw new RuntimeException("My id " + myid + " not in the peer list");
        }
        if (electionType == 0) {//判断选举类型，其实到这个版本这个electionType只有唯一值3了
            try {
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        Election le=null;
        //发现zk其实之前有好多中选举的机制，但是这些在当前版本已经是过期的了
    ...
    @Override
    public void run() {//QuorumPeer本类的run()
        ...
        try {
             ...
             */
            while (running) {
                //紧接着就是去取PeerState，这个PeerState一共有四种：
                // LOOKING：代表还没有确定好角色，就是正在进行领导者选举，如果选举完了，PeerState状态会改变为下面两种之一
                // FOLLOWING：服务器确定是一个follower
                // LEADING：服务器确定是一个leader
                // OBSERVING：服务器确定是一个观察者
                switch (getPeerState()) {
                case LOOKING:
                    LOG.info("LOOKING");
                    //正在寻找leader
                ...
                case FOLLOWING:
                    try {
                        LOG.info("FOLLOWING");
                        //当一台服务器选为Follower后，先new一个Follower
                        setFollower(makeFollower(logFactory));
                        //然后调用followLeader()方法，这个才是核心方法，进入看看会不会发一个请求给leader
                ...
                case LEADING:
                    LOG.info("LEADING");
                    try {
                        //当一台服务器选为leader后，先new一个leader
                        setLeader(makeLeader(logFactory));
                        //然后调用lead()方法，这个才是核心方法，进入看
                        leader.lead();
                        setLeader(null);
                    } catch (Exception e) {
                        LOG.warn("Unexpected exception",e);
    ...
    public Map<Long,QuorumPeer.QuorumServer> getVotingView() {
        return QuorumPeer.viewToVotingView(getView());//返回出去过滤的结果
    }
    ...
    static Map<Long,QuorumPeer.QuorumServer> viewToVotingView(
        ...
        for (QuorumServer server : view.values()) {
            if (server.type == LearnerType.PARTICIPANT) {
                //如果发现LearnerType.PARTICIPANT是这个就放到ret这个map中，最后返回map
