...
public class Learner {       
    ...
    protected void connectToLeader(InetSocketAddress addr, String hostname)
        ...
        for (int tries = 0; tries < 5; tries++) {
            try {
                sock.connect(addr, self.tickTime * self.syncLimit);//给leader地址address，发送一个socket请求，但是这个方法里并没有发送数据
    ...
    protected long registerWithLeader(int pktType) throws IOException{
        ...
        writePacket(qp, true);//写数据，所以learnerHandler就可以读数据了
        readPacket(qp);        //这里的read是Leader端LearnerHandler执行了oa.writeRecord(newEpochPacket, "packet");才开始读取的
    ...
    protected void syncWithLeader(long newLeaderZxid) throws IOException, InterruptedException{
        ...
        synchronized (zk) {
            if (qp.getType() == Leader.DIFF) {//Diff好像什么都没有做
                ...
                zk.getZKDatabase().clear();//首先把当前的数据库清空
                //反序列化leader发回来的数据leaderIs，这个是在connectToLeader()这个方法里拿到的，
                // 其实这两步就是先清空再赋值
                ...
                //这里的truncateLog()就是删数据了，传入的就是超出leader端的事务id
            ...
            outerLoop: //这里加上outerLoop跳出后，就再也不会进入这个循环了
            while (self.isRunning()) {
                ...
                case Leader.PROPOSAL:
                    ...
                    break;
                case Leader.COMMIT:
                    if (!writeToTxnLog) {//需不需要写log，如果不需要就直接更新内存
                    ...
                    } else {//事务就需要写log，就把zxid加到队列packetsCommitted里面，在进行循环，直到
        ...
        } else if (zk instanceof ObserverZooKeeperServer) {//如果是Observer就不会提交这个
