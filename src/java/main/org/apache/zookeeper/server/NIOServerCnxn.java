...
public class NIOServerCnxn extends ServerCnxn {
    ...
    private void readPayload() throws IOException, InterruptedException {
        if (incomingBuffer.remaining() != 0) { // have we read length bytes? 如果没有把incomingBuffer里面的数据都读出来
            int rc = sock.read(incomingBuffer); // sock is non-blocking, so ok 接着读剩下的数据
            if (rc < 0) {//剩下的数据小于0，报错
        ...
        if (incomingBuffer.remaining() == 0) { // have we read length bytes?如果全部读完了，可以请求了
            packetReceived();//计数的方法
            incomingBuffer.flip();
            if (!initialized) {
                readConnectRequest();
            } else {
                //已经连接成功的话，就会到这里，开始读取请求
                readRequest();
            }
            lenBuffer.clear();
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            if (isSocketOpen() == false) {//socket正常
    ...
    private void readRequest() throws IOException {
        //这里服务器实例开始处理收到的packet了，incomingBuffer就是socket里面的数据
