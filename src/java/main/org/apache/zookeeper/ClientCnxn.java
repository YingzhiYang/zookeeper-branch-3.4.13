...
public class ClientCnxn {
    ...
    class EventThread extends ZooKeeperThread {
       ...
       private volatile boolean isRunning = false;
        ...
        public void queueEvent(WatchedEvent event) {
            //WatchedEvent这个类就是事件，如果进去就会看到事件的类型，哪个节点(path)触发了这个事件等等
            ...
            //第一次，什么是pair？这里就是绑定事件，由watcher.materialize绑定一个事件，
            //第二次回来这里我们从Zookeeper.byte[] getData(final String path, Watcher watcher, Stat stat)方法比较好解释，我们现在跳转过去
            //第三次回来，着重看materialize这个方法，我们传入状态，类型，节点等等，
            // 分析watcher.materialize完毕以后我们知道，这个方法返回的就是event的结果而已，而WatcherSetEventPair就是把这个结果封装了一下
            WatcherSetEventPair pair = new WatcherSetEventPair(
                    watcher.materialize(event.getState(), event.getType(),
                            event.getPath()),
                            event);
            // queue the pair (watch set & event) for later processing
            //pair加入队列，但是要说明waitingEvents是个线程队列，所以一定有地方取出来用
            // 我们去找到EventThread.run()方法，要记住此时sendThread和eventThread启动的时候就起来了，就没有停过
       ...
       public void queuePacket(Packet packet) {
        ...
        @Override
        public void run() {//这里就是EventThread的run方法
           try {
              isRunning = true;
              while (true) {
                 Object event = waitingEvents.take();//这里就把上面的pair作为event取出来了
                 if (event == eventOfDeath) {//不是死事件
                    wasKilled = true;
                 } else {
                    processEvent(event);//执行事件，进入，此时要解释怎么绑定事件的，到第二次回来那里
                 }
                 if (wasKilled)
                    synchronized (waitingEvents) {
       ...
       private void processEvent(Object event) {
          try {
              //如果传入的event是刚刚外面封装的WatcherSetEventPair这个pair
              if (event instanceof WatcherSetEventPair) {
                  // each watcher will process the event
                  WatcherSetEventPair pair = (WatcherSetEventPair) event;//那就取出pair
                  for (Watcher watcher : pair.watchers) {//然后再去取返回的pair中所有的watch
                      try {
                          //最后一个一个的触发，而这个process在哪实现的呢，
                          // 其实这个process就是我们自己写的监听器逻辑，比如我们自己写的ZKClient中Watch中的process方法
                          watcher.process(pair.event);//如果你在这里打断点，就会跳到自己写的process方法里
                      } catch (Throwable t) {
    ...
    private void finishPacket(Packet p) {
        //此时这个p已经是服务端传回来的packet，而p.watchRegistration则是其他客户端传递回来的事件
        if (p.watchRegistration != null) {//如果watchRegistration不是null，就把他注册下来，说明其他客户端有变，需要进行事件
            p.watchRegistration.register(p.replyHeader.getErr());//进入register
            //当客户端的事件被注册后又是什么被触发的呢？这个就需要到到客户端xid=-1那里看了，
            // 找到这个ClientCnxn.readResponse()那里if (replyHdr.getXid() == -1)
        }
        //唤醒，然后ClientCnxn.submitRequest可以继续走
        if (p.cb == null) {
            synchronized (p) {
                p.finished = true;
                p.notifyAll();//这里唤醒之前的等待，和之前的也是ClientCnxn.submitRequest方法中的packet.wait()相呼应
            }
        } else {
            p.finished = true;
    ...
    class SendThread extends ZooKeeperThread {
        ...
        private boolean isFirstConnect = true; //第一次连接标记
        ...
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ...
            if (replyHdr.getXid() == -1) {//xid=-1，WatcherEvent的逻辑，执行事件
                ...
                //这里就是接收监听事件的结果的，比如某个客户端修改了数据，其他客户端要感知到就是通过这个WatcherEvent接收的
                WatcherEvent event = new WatcherEvent();
                event.deserialize(bbia, "response");//event反序列化response，这里取出返回的事件
                ...
                //包装成WatchedEvent，注意这里的WatchedEvent和上面的WatcherEvent很像但不是一个类
                WatchedEvent we = new WatchedEvent(event);
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Got " + we + " for sessionid 0x"
                            + Long.toHexString(sessionId));
                }
                //把这个watch event存到队列里面,这里是把服务端告诉我的事件存到这个线程的一个队列里
            ...
            //如果上述都不是，那么说明这个命令是输入的命令行命令，比如create，delete等等
            Packet packet;
            //拿到服务器返回的结果，开始比对本地pendingQueue中的数据，这里就用到了pendingQueue
            synchronized (pendingQueue) {
                if (pendingQueue.size() == 0) {//本地没有数据，说明服务端发错数据了报错
                    throw new IOException("Nothing in the queue, but got "
                            + replyHdr.getXid());
                }
                //取出pendingQueue中的packet，同时删除
            ...
            try {
                ...
                //EvenThread讲解：最终到finally这里，这里面有唤醒语句，和之前的ClientCnxn.submitRequest方法中的packet.wait()相呼应。
                finishPacket(packet);
            }
        }
        ...
        void primeConnection() throws IOException {
            LOG.info("Socket connection established to "
                     ...
                     + ", initiating session");
            isFirstConnect = false;//修改首次连接标记，就会设置为false，执行
            long sessId = (seenRwServerBefore) ? sessionId : 0;
            ConnectRequest conReq = new ConnectRequest(0, lastZxid,
                    sessionTimeout, sessId, sessionPasswd);//初始化连接请求
        ...
        private void startConnect(InetSocketAddress addr) throws IOException {
            // initializing it for new connection 开始连接了
        ...
        //这个run方法是SendThread这个内部类中的，由ClientCnxn中的cnxn.start()中的sendThread.start()启动的
        @Override
        public void run() {
            ...
            while (state.isAlive()) {//这里也是重试机制的地方，使用的while，try catch和flag一起实现的
                try {
                    //如果socket还没有连接，往下走开始连接
                    if (!clientCnxnSocket.isConnected()) {
                        //如果不是第一次连接走到if内，如果是第一次连接往下，isFirstConnect默认是true
                        if(!isFirstConnect){
                            try {
                        ...
                        //第一次closing默认false，state alive加非也是false，往下
                        ...
                        if (rwServerAddress != null) {
                            ...
                            //第一次连接，拿到一个hostProvider,Host provider在前面初始化，随机分配一个地址StaticHostProvider.Collections.shuffle
                            serverAddress = hostProvider.next(1000);//这里有一个避免取失败host的小操作
                        }
                        //进行socket连接这个地址
                        startConnect(serverAddress);//这个方法里修改isFirstConnect的值，以判断第一次连接过了
                        clientCnxnSocket.updateLastSendAndHeard();
                    }
                    ...
                    if (to <= 0) {
                        ...
                        warnInfo = "Client session timed out, have not heard from server in "
                            ...
                            + Long.toHexString(sessionId);
                        LOG.warn(warnInfo);
                        throw new SessionTimeoutException(warnInfo);//这里会爆出异常，然后被外面的try捕获，转到下面的catch
                    }
                    //没有超时，连接成功
                    if (state.isConnected()) {
                        ...
                        if (timeToNextPing <= 0 || clientCnxnSocket.getIdleSend() > MAX_SEND_PING_INTERVAL) {
                            //发送ping给客户端，由于while循环会一直进行下去，所以这里的ping会一直发送给sever确认状态
                            //这里发送ping的代码和输入的命令行一模一样也是加到了outgoingQueue里
                            sendPing();
                    ...
                    if (closing) {
                        ...
                        if (e instanceof SessionExpiredException) {
                            ...
                            LOG.info(e.getMessage() + RETRY_CONN_MSG);//超时异常打一个log，继续走最外面的while语句，这里就是重试机制
    ...
    public void close() throws IOException {
        ...
        try {
            ...
            //又见到一个submitRequest，这里就是提交了closeSession这个操作给了服务端，
            // 我们转到服务端看看是怎么处理CloseSession的，我们直接去看请求链的第一个PrepRequestProcessor
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        //最终把请求包装成为一个包(packet)队列等待被消费，进入方法可看到这个队列是被synchronized
        //包括着的，是一个强锁。这个queue就是outgoingQueue，这个queue存放的是就是正在要被提交的请求
        //这里的watchRegistration虽然回传给服务端，但是服务端并不会使用这个，
        // 而是把watchRegistration传递给客户端们，我们下面可以进入queuePacket看下
        Packet packet = queuePacket(h, r, request, response, null, null, null,
                    null, watchRegistration);
        //提交后应该等待服务端的结果，因为要从服务端拿到request和response，所以这里有一个synchronized的阻塞队列
        //这里会一直被阻塞，直到服务端把结果告诉“我”然后把拿到数据的ReplyHeader对象返回
        synchronized (packet) {
            while (!packet.finished) {
                //这里把packet等待，所以必然有一处要唤醒packet，此时等待Sever端传来数据
                //最终这里会由ClientCnxn中的SendThread接到返回后在finishPacket()方法中唤醒
                packet.wait();

            }
        }
        //当收到返回有值的时候在返回r(ReplyHeader实例对象)
        return r;
    }
    ...
    {//进入后我们看下面
        ...
        synchronized (outgoingQueue) {//什么时候取出outgoingQueue里的数据呢
            //EventThread介绍：这里我们把watchRegistration又new成了packet最后返回出去，并且通过queue传递给服务端，
            // 我们最后会在接收结果的时候使用这个packet，
            //这个packet是谁使用的呢，还是SendThread这个收发通用的线程在用，我们找到SendThread.readResponse().finishPacket(packet);
