...
public class ZooKeeper {
    ...
    private static class ZKWatchManager implements ClientWatchManager {
                                        ...
                                        String clientPath)
        {
            ...
            switch (type) {//这里开始区分是哪个事件
            case None://空事件，不改变，或者初始化会用到。往下走
            ...
            case NodeCreated://NodeCreated，NodeDataChanged事件，进入逻辑
                synchronized (dataWatches) {
                    //dataWatches这个map就是刚刚我们看的那个注册的map
                    //从watch的map中移除并拿到node(clientPath)上面绑定的事件(watch)，并且存到result这个set中
                    addTo(dataWatches.remove(clientPath), result);//然后我们找下result做了什么，往下走
                    //而且这里的remove也是为什么原生的客户端的watch只能被触发一次，因为用了就被remove，
                    // 假设我们用get我们也许能一直用下去，但是事件会重复
                }
                synchronized (existWatches) {
            ...
            //result直接返回出去，也就是把clientPath上绑定的事件直接返回了出去
            return result;
        }
    }
    ...
    abstract class WatchRegistration {
        ...
        public void register(int rc) {
            //进入以后我们发现，所有的watch都被存到了一个map里，这个map在哪里用呢？先放在这里
            // 其实是在ClientCnxn中被eventThread调用起来了，在queueEvent()中watcher.materialize()中使用的,
            // 为了构造WatcherSetEventPair pair这个对象，materialize()这个方法其实就是在本类中实现的，我们转过去
        ...
        throws IOException
    {
        ...
        //拿到一个NIO的client,getClientCnxnSocket()，也是在这里面new出来SendThread和EventThread的
    ...
    public synchronized void close() throws InterruptedException {
        if (!cnxn.getState().isAlive()) {//客户端验证存活，往下
        ...
        try {
            cnxn.close();//重点在这里，cnxn调用了close，进入
        } catch (IOException e) {
            if (LOG.isDebugEnabled()) {
        ...
        throws KeeperException, InterruptedException
    {
        ...
        //判断是不是顺序节点，只是一个验证，验证是否是顺序节点(createMode.isSequential())，是否路径非法
        ...
        request.setAcl(acl);
        //提交请求，通过socket传递到服务器端，拿到响应头，所谓提交请求，就是把请求包装成一个packet然后封装到一个队列中去
    ...
    public byte[] getData(final String path, Watcher watcher, Stat stat)
        throws KeeperException, InterruptedException
     {
        ...
        request.setPath(serverPath);
         //这个watch只是判断了一下是不是有的，只是一个true/false并没有传入实际的对象，那么为什么要把有没有watch告诉服务端呢？
         // 这里要联合服务端去看，我们一会儿去看，先记住这里
         // 但是这也说明了客户端并不需要把整个watch都给服务器
        request.setWatch(watcher != null);
        GetDataResponse response = new GetDataResponse();
         //传入submit，在这里封装成packet传给服务器，进入
        ReplyHeader r = cnxn.submitRequest(h, request, response, wcb);
        if (r.getErr() != 0) {
            throw KeeperException.create(KeeperException.Code.get(r.getErr()),
                    clientPath);
    ...
    public byte[] getData(String path, boolean watch, Stat stat)
            throws KeeperException, InterruptedException {
        //从ClientCnxn.queueEvent().WatcherSetEventPair跳转来，继续进入getData
    ...
    //这个方法就是拿到ClientCnxnSocket这个抽象类的实例
    private static ClientCnxnSocket getClientCnxnSocket() throws IOException {
        //系统自定义的包名
        String clientCnxnSocketName = System
                .getProperty(ZOOKEEPER_CLIENT_CNXN_SOCKET);
        //如果拿不到，那么从ClientCnxnSocketNIO这里把类拿出来，
        //因为ClientCnxnSocketNIO是ClientCnxnSocket这个抽象类的实现类，上面的拿到了
        // 也是ClientCnxnSocketNIO真正处理的
        if (clientCnxnSocketName == null) {
            clientCnxnSocketName = ClientCnxnSocketNIO.class.getName();
        }
        try {
            //把ClientCnxnSocket实例对象返回，此处可以看到，zk连接使用的socket对象其实是ClientCnxnSocketNIO提供的
